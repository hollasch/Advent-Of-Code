//----------------------------------------------------------------------------------------------------------------------
// 2023 Advent of Code -- Puzzle 06b (See Part Two below)
//
// --- Day 6: Wait For It ---
//
...
// "Boat races! Open to the public! Grand prize is an all-expenses-paid trip to Desert Island!" That
// must be where the sand comes from! Best of all, the boat races are starting in just a few
// minutes.
//
// You manage to sign up as a competitor in the boat races just in time. The organizer explains that
// it's not really a traditional race - instead, you will get a fixed amount of time during which
// your boat has to travel as far as it can, and you win if your boat goes the farthest.
//
...
//     of 6 millimeters. - Hold the button for 2 milliseconds, giving the boat a speed of 2
//     millimeters per millisecond. It will then get 5 milliseconds to move, reaching a total
...
RaceData readRaceData() {
    ...
    for(auto sit = line.begin() + 5;  sit < line.end();  ++sit) {
        if (*sit != ' ')
            timeString += *sit;
    }
    ...
    for (auto sit = line.begin() + 9;  sit < line.end();  ++sit) {
        if (*sit != ' ')
            recordString += *sit;
    }

    return { stod(timeString), stod(recordString) };
}


int main() {
    RaceData raceData = readRaceData();

    unsigned long long numWays = 0;

    cout << "Time " << raceData.length << ", distance " << raceData.record << "\n";

    // Determine the minimum time to press the charge button to beat the record.
    double minPress = (raceData.length - sqrt(raceData.length*raceData.length - 4*raceData.record)) / 2;

    if (minPress == ceil(minPress)) // Matching the current record exactly is not enough.
        ++minPress;

    minPress = ceil(minPress);
    double distance = minPress * (raceData.length - minPress);
    if (distance > raceData.record)
        numWays = (raceData.length - 2*minPress + 1);
